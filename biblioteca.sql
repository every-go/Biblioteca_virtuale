DROP TABLE IF EXISTS Manga;
DROP TABLE IF EXISTS Libro;
DROP TABLE IF EXISTS Rivista;
DROP TABLE IF EXISTS Film;
DROP TABLE IF EXISTS CD;
DROP TABLE IF EXISTS Multimedia;
DROP TABLE IF EXISTS Cartaceo;
DROP TABLE IF EXISTS Biblioteca;
DROP TYPE IF EXISTS Diffusione;

CREATE TABLE Biblioteca (
   id SERIAL PRIMARY KEY,
   titolo TEXT,
   genere TEXT,
   anno INTEGER,
   costo DECIMAL(10,2),
   disponibile BOOLEAN NOT NULL,
   copie_disponibili INTEGER NOT NULL,
   nprestiti INTEGER NOT NULL,
   immagine TEXT,
   CHECK(costo >= 0 AND copie_disponibili >= 0 AND nprestiti >= 0 AND nprestiti >= copie_disponibili)
);

CREATE TABLE Multimedia(
   id INTEGER PRIMARY KEY,
   durata INTEGER,
   studio TEXT,
   FOREIGN KEY (id) REFERENCES Biblioteca(id) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE Cartaceo(
   id INTEGER PRIMARY KEY,
   autore TEXT,
   editore TEXT,
   letto BOOLEAN NOT NULL,
   FOREIGN KEY (id) REFERENCES Biblioteca(id) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE Film(
   id INTEGER PRIMARY KEY,
   regista TEXT,
   attore_protagonista TEXT,
   visto BOOLEAN NOT NULL,
   FOREIGN KEY (id) REFERENCES Multimedia(id) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE CD(
   id INTEGER PRIMARY KEY,
   artista TEXT,
   genere TEXT,
   ntracce INTEGER NOT NULL,
   FOREIGN KEY (id) REFERENCES Multimedia(id) ON DELETE CASCADE ON UPDATE CASCADE,
   CHECK (ntracce > 0)
);

CREATE TYPE Diffusione AS ENUM ('Provinciale', 'Regionale', 'Nazionale', 'Internazionale');

CREATE TABLE Rivista(
   id INTEGER PRIMARY KEY,
   diffusion Diffusione NOT NULL,
   FOREIGN KEY (id) REFERENCES Cartaceo(id) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE Libro(
   id INTEGER PRIMARY KEY,
   lingua_originale TEXT,
   nvolume INTEGER NOT NULL,
   FOREIGN KEY (id) REFERENCES Cartaceo(id) ON DELETE CASCADE ON UPDATE CASCADE,
   CHECK (nvolume > 0)
);

CREATE TABLE Manga(
   id INTEGER PRIMARY KEY,
   concluso BOOLEAN NOT NULL,
   FOREIGN KEY (id) REFERENCES Libro(id) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE OR REPLACE FUNCTION aggiorna_disponibile() RETURNS TRIGGER AS $$
BEGIN
   IF NEW.nprestiti = 0 THEN
      NEW.disponibile := FALSE;
   ELSE
      NEW.disponibile := TRUE;
   END IF;
   RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_aggiorna_disponibile
BEFORE UPDATE OF nprestiti ON Biblioteca
FOR EACH ROW
EXECUTE FUNCTION aggiorna_disponibile();

CREATE OR REPLACE FUNCTION controlla_prestiti_copie() RETURNS TRIGGER AS $$
BEGIN
   IF NEW.nprestiti < 0 OR NEW.nprestiti < NEW.copie_disponibili THEN
      RAISE EXCEPTION 'Il numero di prestiti possibili (prenotati+prenotabili) deve essere >= 0 e >= copie disponibili';
   END IF;
   RETURN NEW;
END;
$$ LANGUAGE plpgsql;


CREATE TRIGGER trg_controlla_prestiti_copie
BEFORE UPDATE OF nprestiti ON Biblioteca
FOR EACH ROW
EXECUTE FUNCTION controlla_prestiti_copie();


-- ID 1: Film
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (1, 'Ace Ventura', 'Commedia', 1994, 9.99, TRUE, 1, 1, 'IMG/ace_ventura.png');
INSERT INTO Multimedia(id, durata, studio)
VALUES (1, 86, 'Warner Bros');
INSERT INTO Film(id, regista, attore_protagonista, visto)
VALUES (1, 'Tom Shadyac', 'Jim Carrey', FALSE);

-- ID 2: Manga
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (2, 'Bleach', 'Shonen', 2001, 5.20, TRUE, 3, 3, ':/images/bleach.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (2, 'Tite Kubo', 'Shueisha', TRUE);
INSERT INTO Libro(id, lingua_originale, nvolume)
VALUES (2, 'Giapponese', 69);
INSERT INTO Manga(id, concluso)
VALUES (2, TRUE);

-- ID 3: Riviste
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (3, 'Cook_inc', 'Culinario', 2011, 3.99, TRUE, 3, 3, 'IMG/cook_inc.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (3, 'Luigi Nardi', 'Vandenberg Edizioni', FALSE);
INSERT INTO Rivista(id, diffusion)
VALUES (3, 'Regionale');

-- ID 4: Riviste
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (4, 'Focus', 'Scientifico', 1992, 3.99, TRUE, 4, 4, ':/images/focus.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (4, 'Raffaele Leone', 'Mondadori', FALSE);
INSERT INTO Rivista(id, diffusion)
VALUES (4, 'Nazionale');

-- ID 5: Libri
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (5, 'Harry Potter', 'Romanzo', 1997, 7.99, TRUE, 5, 5, ':/images/harry_potter.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (5, 'J. K. Rowling', 'Bloomsbury Publishing', FALSE);
INSERT INTO Libro(id, lingua_originale, nvolume)
VALUES (5, 'Inglese', 7);

-- ID 6: Libri
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (6, 'I ragazzi della via Pal', 'Romanzo', 1906, 7.99, TRUE, 5, 5, ':/images/via_pal.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (6, 'Ferenc Molnár', 'Giunti', TRUE);
INSERT INTO Libro(id, lingua_originale, nvolume)
VALUES (6, 'Ungherese', 1);

-- ID 7: Libri (duplicato titolo ma differente immagine)
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (7, 'I ragazzi della via Pal', 'Romanzo', 1906, 7.99, TRUE, 5, 5, ':/images/via_pal2.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (7, 'Ferenc Molnár', 'Giunti', TRUE);
INSERT INTO Libro(id, lingua_originale, nvolume)
VALUES (7, 'Ungherese', 1);

-- ID 8: Libri
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (8, 'L''isola del tesoro', 'Romanzo', 1883, 7.99, TRUE, 3, 3, ':/images/isola_tesoro.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (8, 'Robert Louis Stevenson', 'Giunti', TRUE);
INSERT INTO Libro(id, lingua_originale, nvolume)
VALUES (8, 'Inglese', 1);

-- ID 9: CD
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (9, 'La Divina Commedia', 'Rap', 2023, 8.99, TRUE, 4, 4, ':/images/la_divina_commedia2.png');
INSERT INTO Multimedia(id, durata, studio)
VALUES (9, 49, 'Sony');
INSERT INTO CD(id, artista, genere, ntracce)
VALUES (9, 'Tedua', 'Rap', 16);

-- ID 10: CD (duplicato titolo con immagine diversa)
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (10, 'La Divina Commedia', 'Rap', 2023, 8.99, TRUE, 4, 4, ':/images/la_divina_commedia.png');
INSERT INTO Multimedia(id, durata, studio)
VALUES (10, 49, 'Sony');
INSERT INTO CD(id, artista, genere, ntracce)
VALUES (10, 'Tedua', 'Rap', 16);

-- ID 11: Libri
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (11, 'La Divina Commedia', 'Poema', 1472, 9.99, TRUE, 1472, 1472, ':/images/divina.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (11, 'Dante Alighieri', '', FALSE);
INSERT INTO Libro(id, lingua_originale, nvolume)
VALUES (11, 'Volgare fiorentino', 1);

-- ID 12: Manga
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (12, 'One Piece', 'Shonen', 1999, 5.20, TRUE, 3, 3, ':/images/onepiece.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (12, 'Eiichiro Oda', 'Shueisha', TRUE);
INSERT INTO Libro(id, lingua_originale, nvolume)
VALUES (12, 'Giapponese', 110);
INSERT INTO Manga(id, concluso)
VALUES (12, FALSE);

-- ID 13: Film
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (13, 'Shrek', 'Animazione', 2001, 9.99, TRUE, 1, 1, ':/images/shrek.png');
INSERT INTO Multimedia(id, durata, studio)
VALUES (13, 90, 'DreamWorks Animation');
INSERT INTO Film(id, regista, attore_protagonista, visto)
VALUES (13, 'Andrew Adamson', 'Shrek', FALSE);

-- ID 14: Film
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (14, 'Skyscraper', 'Avventura', 2018, 9.99, TRUE, 1, 1, ':/images/skyscraper.png');
INSERT INTO Multimedia(id, durata, studio)
VALUES (14, 102, 'Legendary Pictures');
INSERT INTO Film(id, regista, attore_protagonista, visto)
VALUES (14, 'Rawson Marshall Thurber', 'Dwayne Johnson', TRUE);

-- ID 15: CD
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (15, 'The dark side of the moon', 'Rock', 1973, 8.99, TRUE, 5, 5, ':/images/dark_side.png');
INSERT INTO Multimedia(id, durata, studio)
VALUES (15, 43, 'Harvest');
INSERT INTO CD(id, artista, genere, ntracce)
VALUES (15, 'Pink Floyd', 'Rock', 10);

-- ID 16: Riviste
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (16, 'Vanity fair', 'Moda', 2003, 3.99, TRUE, 4, 5, ':/images/vanity_fair.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (16, 'Simone Marchetti', 'Condé Nast', FALSE);
INSERT INTO Rivista(id, diffusion)
VALUES (16, 'Nazionale');

-- ID 17: Riviste (duplicato)
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (17, 'Vanity fair', 'Moda', 2003, 3.99, TRUE, 5, 5, ':/images/vanity_fair2.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (17, 'Simone Marchetti', 'Condé Nast', FALSE);
INSERT INTO Rivista(id, diffusion)
VALUES (17, 'Nazionale');

-- ID 18: Manga
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (18, 'Vinland Saga Vol.1', 'Seinen', 2005, 5.20, TRUE, 3, 3, ':/images/vinland_saga2.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (18, 'Makoto Yukimura', 'Kodansha', TRUE);
INSERT INTO Libro(id, lingua_originale, nvolume)
VALUES (18, 'Giapponese', 28);
INSERT INTO Manga(id, concluso)
VALUES (18, FALSE);

-- ID 19: Manga (presumo errore nome autore "Yukimira" invece di Yukimura, mantengo come da file)
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (19, 'Vinland Saga Vol.2', 'Seinen', 2006, 5.20, TRUE, 3, 3, ':/images/vinland_saga.png');
INSERT INTO Cartaceo(id, autore, editore, letto)
VALUES (19, 'Makoto Yukimira', 'Kodansha', TRUE);
INSERT INTO Libro(id, lingua_originale, nvolume)
VALUES (19, 'Giapponese', 28);
INSERT INTO Manga(id, concluso)
VALUES (19, FALSE);

-- ID 20: CD
INSERT INTO Biblioteca(id, titolo, genere, anno, costo, disponibile, copie_disponibili, nprestiti, immagine)
VALUES (20, 'È finita la pace', 'Rap', 2024, 8.99, TRUE, 3, 5, ':/images/e_finita_la_pace.png');
INSERT INTO Multimedia(id, durata, studio)
VALUES (20, 46, 'Island');
INSERT INTO CD(id, artista, genere, ntracce)
VALUES (20, 'Marracash', 'Rap', 13);

